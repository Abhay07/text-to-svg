...
export default class TextToSVG {
  ...
  getD(text, options = {}) {
    ...
    const paths = this.font.getPaths(text, metrics.x, metrics.baseline, fontSize, { kerning, letterSpacing, tracking });
    const pathDatas = [];
    paths.forEach((n) => {
  ...
  getPath(text, options = {}) {
    ...
    d.forEach((n,i) => {
      if (attributes) {
        paths = `${paths}
          <path ${attributes} class="textPath textPath${i}" d="${n}"/>
        `
      }
      else {
        paths = `${paths}
          <path d="${n}" class="textPath textPath${i}" fill="red" stroke="black"/>
        `;
      }
    });
